...
export const BASE64_NUMBER_OF_BITS = 6;
export const BASE64_ZEROS_CHAR = BASE64_CHARS[0]; // 000000 in bits
export const BASE64_ONES_CHAR = BASE64_CHARS[BASE64_LEN-1]; // 111111 in bits
...
export function binaryStringToBase64String(str: string): string {
  let ret = "";
  const strChunked = chunkString(str, BASE64_NUMBER_OF_BITS);
...
export function base64StringToBinaryString(str: string): string {
  let ret = "";
  const zeroPad = "0".repeat(BASE64_NUMBER_OF_BITS);
  for (let i = 0; i < str.length; i++) {
    ret += (zeroPad + base64ToBinary(str[i])).slice(-BASE64_NUMBER_OF_BITS);
  }
  return ret;
}