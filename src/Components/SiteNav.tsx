...
export default function SiteNav({activatePage} : Readonly<Props>) {
  const linkClassName = "px-5 hover:text-white py-1 font-bold rounded whitespace-nowrap transition-colors duration-200 ";
  ...
  return (
      <nav className="pt-1 h-10 bg-blue-400">
        <div className="flex">
          ...
          <Link className={linkClassName + "ml-auto " + (activatePage == PagesEnum.OPTIONS ? "text-white" : "")} href={OPTIONS_PAGE_PATH}>Options</Link>
        </div>
      </nav>
  );
}