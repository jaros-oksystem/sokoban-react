...
export function splitStringGroupsWhichExceedMaximumLength(groups: string[], maxLen: number): string[] {
  const ret: string[] = [];
  for (let i = 0; i < groups.length; i++) {
    if (groups[i].length <= maxLen) {
      ret.push(groups[i]);
    } else {
      const newGroups = chunkString(groups[i], maxLen);
      for (let j = 0; j < newGroups.length; j++) {
        ret.push(newGroups[j]);
      }
    }
  }
  return ret;
